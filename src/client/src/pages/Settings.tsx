...
import { getWebhookConfig, updateWebhookConfig, getNtfyConfig, updateNtfyConfig, testNtfyNotification, getMyToken, resetMyToken, api } from '../api'
...
export function Settings() {
  ...
  useEffect(() => {
    ...
    getMyToken().then(setApiToken).catch(() => {})
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-8">
      ...
      {/* Token API */}
      <section className="bg-[var(--bg-secondary)] rounded-xl p-6 space-y-4">
        <h2 className="text-lg font-semibold text-white">Token API</h2>
        <p className="text-sm text-[var(--text-secondary)]">
          Utilisez ce token pour acceder a l'API depuis des outils externes (curl, Raccourcis, etc.).
        </p>

        {apiToken ? (
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-[var(--text-secondary)] mb-2">Votre token</label>
              <div className="relative">
                <input
                  type={showApiToken ? 'text' : 'password'}
                  value={apiToken}
                  readOnly
                  className="w-full px-3 py-2 pr-10 bg-[var(--bg-tertiary)] rounded-lg text-white text-sm focus:outline-none font-mono"
                />
                <button
                  type="button"
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-[var(--bg-tertiary)] text-[var(--text-secondary)] rounded-lg text-sm hover:text-white transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                </svg>
                {apiTokenCopied ? 'Copie!' : 'Copier'}
              </button>
              <button
                onClick={async () => {
                  setApiTokenLoading(true)
                  try {
                    const newToken = await resetMyToken()
            ...
            <div className="flex items-start gap-3 p-3 bg-amber-500/10 rounded-lg">
              ...
              <p className="text-sm text-amber-300/80">
                Regenerer le token invalidera tous vos acces existants.
              </p>
            </div>
          </div>
        ) : (
            ...
            {webhookConfig.requiresToken && (
              <div className="space-y-4">
                ...
                <div className="p-3 bg-[var(--bg-tertiary)] rounded-lg">
                  <p className="text-sm text-[var(--text-secondary)]">
                    Utilisez le token de la section <span className="text-white font-medium">Token API</span> ci-dessus comme header <code className="text-[var(--accent)]">X-Webhook-Token</code>.
                  </p>
                </div>
              </div>
            )}
